...
public class MyUserDetailsService implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		if(account!=null) {
			...
			LOGGER.error("{} is not found while build user for authentication",username,e);
			throw e;
		}
	...
	private UserDetails buildUserForAuthentication(Account account, List<GrantedAuthority> authorities) {
		return new User(account.getUsername(), account.getPassword(), account.getStatus().equals("active"), true, true, true, authorities);
	}
